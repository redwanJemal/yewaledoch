...
export function MyPostsPage({ onBack, onPostTap }: MyPostsPageProps) {
  ...
  useEffect(() => {
    postsApi.mine({ page: 1, per_page: 20 })
      .then((res) => {
        setPosts(res.items);
  ...
  const loadMore = () => {
    const next = page + 1;
    setPage(next);
    postsApi.mine({ page: next, per_page: 20 })
      .then((res) => {
  ...
  return (
    <div className="min-h-screen bg-tg-bg">
      <div className="sticky top-0 z-10 bg-tg-bg border-b border-tg-hint/10 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-tg-text">{t('profile.my_posts')}</h1>
      </div>
      ...
      <div className="p-4 space-y-3">
        {loading ? (
          ...
          <EmptyState icon="📝" message={t('empty.posts')} />
        ) : (
          <>
            {posts.map((post) => (
